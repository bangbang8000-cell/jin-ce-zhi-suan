...
class WebhookNotifier:
    ...
    def _build_text(self, event_type, stock_code, data):
        ...
        if amount_val is not None:
            amount_label = "硅基金额" if direction == "BUY" else ("流码金额" if direction == "SELL" else "成交金额")
    ...
    def _build_feishu_payload(self, event_type, stock_code, data):
        ...
        if not isinstance(data, dict):
            text_body = (
                ...
                f"**祖宗**：`{self._safe_text(stock_code)}`\n"
        ...
        if event_type == "system":
            ...
            scope_text = "全局"
            if codes:
                scope_text = f"{len(codes)}只祖宗：{'、'.join(codes)}" if len(codes) > 1 else codes[0]
            lines = [
